    ...
    try {
      ...
      for(order_array of order) {
        ...
        while((retry < number_of_trials) && !success) {
          ...
          server.send('192.168.0.139', 15000, result, function (err, ackData) {
        ...
        if(!success) {
          //Sending Failed
          status_json = {"sending_status": "FAILED", "result": result, "code": 400}
        } else {
          status_json = {"sending_status": "SUCCESS", "result": result, "code": 200}
        }

        responseBody = JSON.stringify(status_json)
        // set content type header so that OpenHIM knows how to handle the response
        res.set('Content-Type', 'application/json+openhim')

        // construct return object
        var properties = { property: 'Order Route' }
        res.send(utils.buildReturnObject(mediatorConfig.urn, status_json.sending_status, status_json.code, headers, responseBody, 
                                  orchestrations, properties))
        result_array.push(result)
      }

      winston.info(JSON.parse(result_array))
      res.send(JSON.parse(result_array))
      return
    } catch (err) {
