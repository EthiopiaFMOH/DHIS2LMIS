...
function setupApp () {
  ...
  var server = new mllp.MLLPServer('0.0.0.0', 4422)
  ...
  app.post('/order', async (req, res) => {
    ...
    let orchestrations = []
    let order_data, order_array, order, result = '', result_array = [], error_suspect_id = []
    try {
      order = req.body
      ...
      for(order_array of order) {
        ...
        try{
          ...
          server.send('196.189.117.34', 15000, result, await function (err, ackData) {
            
            // async callback code here
            if(err) {
              error_suspect_id.push = order_array[mediatorConfig.config.externalpatientid]
            }
            console.log('err:', err)
  ...
  app.post('/result', async (req, res) => {
    ...
    try {
      ...
      let result_pid = '"suspect_id": "' + pid_array[indexes.suspect_id] + '", "last_name": "' + pid_array[indexes.name] + '", "date_of_birth": "' +
	...
	console.log('test 2')
      ...
      console.log('_____________________Result_______________________');
      ...
      console.log('_____________________Result JSON_______________________');
      ...
      let return_data
      try{
        let insert_detail = await fetch(mediatorConfig.config.DHIS2baseurl + labTestResult, {
          method: "POST",
          headers: {
      ...
      var responseBody = return_data
      console.log('Responde body ' + responseBody)
